...
\begin{definition}[Tangent]
  ...
  \begin{itemize}
    ...
    $$\dv{x^i}{t}(\sigma_1(t))\Bigg|_{t=0} =\dv{x^i}{t}(\sigma_2(t))\Bigg|_{t=0}  \ \forall \ i = 1,2,\ldots,m$$
  \end{itemize}
...
We can also show that $T_p\mathcal{M}$ and $D_p\mathcal{M}$ are actually isomorphic and thus, both approaches can be taken to describe the tangent-space structure (However, the algebraic approach is a bit problematic for infinite-dimensional space). 
...
These quantitites form a basis set for the vector space $T_p\mathcal{M}$. For that we have to check for linear independence and span of these quantitites. 
\begin{proposition}
