...
\end{itemize}
We thus see that for $\Gamma^a, \Gamma^b \in \Gamma$, $\Tr(\Gamma^a \Gamma^b) \neq 0$ only when $\Gamma^a = \Gamma^b$ where the equality means that both are same kind of matrices in the Clifford algebra basis. With this, we define $$\Gamma_{a} = \frac{\Gamma^a}{\Tr(\Gamma^a\Gamma^a)}\qquad \{ \Gamma_{a}\} = \{\iden, \gamma^\mu, -\gamma^\mu\gamma^5, \Sigma^{\mu\nu}, \gamma^5\} $$
where the index is not summed in the trace. Then we can say:
$$\Tr(\Gamma_a \Gamma^b) = \delt{b}{a}$$
...
$$\Tr\Gamma_b M = \sum\limits_a C^M_a \Tr \Gamma_b\Gamma^a = \sum\limits_a C^M_a \delt{a}{b} = C^M_b \implies \boxed{ C^M_b = \Tr\Gamma_b M}$$
And hence the matrix is expanded as:
\begin{align}\label{eqn:expand}
    M = \sum\limits_a \Tr(\Gamma_a M)\ \Gamma^a
\end{align}
...
\begin{equation}\label{eqn:complete}
    M_{\alpha\beta} = \sum\limits_{a} (\Gamma_{a})_{\eta \lambda}M_{\lambda \eta} {\Gamma^a}_{\alpha\beta} = \sum\limits_{a} \qty{(\Gamma_{a})_{\eta \lambda} {(\Gamma^a)}_{\alpha\beta}} M_{\lambda \eta}\implies \boxed{\sum\limits_a (\Gamma_{a})_{\eta \lambda} {(\Gamma^a)}_{\alpha\beta} = \delta_{\alpha\lambda}\delta_{\beta\eta}}
\end{equation}
