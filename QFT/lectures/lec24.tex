...
$$\boxed{\SL\tund{D} = \dadj{\Psi}\ (i \gsl{\partial} - m)\ \Psi = \dadj{\Psi}\ (i \gamma^\mu \partial_\mu - m)\ \Psi}$$
...
%
   ...
   The Dirac Lagrangian exhibits a global $\mathrm{U}(1)$ symmetry since if we consider $$\Psi \longrightarrow e^{i\alpha}\Psi \implies \dadj{\Psi} \longrightarrow e^{-i\alpha}\dadj{\Psi}$$Then, 
   $$\SL_{\mathrm{D}} \longrightarrow \SL_{\mathrm{D}}' = i e^{-i\alpha}\dadj{\Psi} \gamma^\mu e^{i\alpha}\Psi - m e^{-i\alpha}e^{i\alpha}\dadj{\Psi}\Psi =\SL$$
   Under the infinitesimal transformation of the Dirac field,
   $$\delta \Psi = \Psi' - \Psi = \qty(e^{i\alpha}-1)\Psi = [(1 + i\alpha) -1]\Psi = i\alpha\Psi$$
   using Eq. \ref{eqn:noethercurr} we can find the conserved Noether current as,
   \begin{align}\label{eqn:diracnoether}
      J^\mu = \pdv{\SL_{\mathrm{D}}}{(\partial_\mu \Psi)}\delta\Psi + \cancelto{0}{\pdv{\SL_{\mathrm{D}}}{(\partial_\mu \dadj{\Psi})}\delta\dadj{\Psi}} = (i\dadj{\Psi}\gamma^\mu)(i\alpha\Psi) = -\dadj{\Psi}\gamma^\mu\Psi
   \end{align}
...
Till now we have not talked about any commutation relations that is neccessary for quantisation. In general, we assume the field components indexed by $\{\alpha\}$ running from $1$ to $4$  (since the spinors are basically column vectors) and with our previous intuition, let us guess the commutation relations between the field $\Psi_{\alpha}(\vb{x})$ and the conjugate momentum $i\Psi_{\alpha}^\dagger(\vb{x})$,
...
\begin{align}\label{eqn:creationcommut}
   ...
   \end{split}
\end{align}
with all other commutators vanishing. We now check whether using the above relations, we recover the field commutations from Eq. \ref{eqn:fieldcommutation} or not. For simplicity, we will suppress the index for the components of the field.
\begin{align*}
...
\end{align*}
where in the above steps, we have used Eq. \ref{eqn:spinsum} and in the second last step, in the second term, we changed $\vb{p}\to -\vb{p}$ in the integral, so $p_i \to -p_i$ accordingly. We thus see that assuming these form of creation/annihilation operators yields the correct field quantisation.\footnote{Technically, the commutation result should give $$\comtr{\Psi_\alpha(\vb{x})}{\Psi_\beta^\dagger(\vb{y})} =\delta_{\alpha \beta} \delta^{(3)}(\vb{x}-\vb{y})$$ if we have used the index for the field components. Hence the commutation relation between the fields is proportional to the identity matrix.} \\[0.2cm]
Now, note that we see an inocuous $\mathcolor{red}{-}$ sign in the Hamiltonian (Eq. \ref{eqn:hamiltonian}) which imply that the spectrum is unbounded from below. If we create more and more particles with $\hat{b}^\dagger$, we can lower the energy indefinitely which is really, really \textit{unphysical}. We could instead make a reinterpretation as $\hat{b}^\dagger \leftrightarrow \hat{b}$ but then the commutation relations in Eq. \ref{eqn:creationcommut} gets ruined and the states now get negative norm. \\[0.2cm]
