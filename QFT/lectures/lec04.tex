...
From this, the Lagrangian of the system is obtained as:
...
Position and time, both are kind of labels and thus are independent. Hence, total and partial derivatives coincide in this case. Note that in continuum, the Lagrangian itself becomes an integral of the form:
$$L = \frac{1}{2}\int \qty[\mu \dot{\eta}^2 - Y \qty(\dv{\eta}{x})^2] dx$$
The bracketted term is the Lagrangian density, denoted by $\SL$ and hence, the Lagrangian is the space integral of the Lagrangian density: $$L = \int \SL \ dx$$
Henceforth, we allow ourselves to be \textit{sloppy} enough and refer to Lagrangian density as simply the Lagrangian when context is clear.
\subsection{Our actions tell a lot!}
...
$$S = \int\limits_{t_1}^{t_2} dt \ L = \int\limits_{t_1}^{t_2} dt \int d^3\vb{x}\ \SL(\phi, \partial_\mu \phi, x^\mu) = \int\limits_{\SR\subset \re^4} d^4\vb{x} \ \SL(\phi, \partial_\mu \phi, x^\mu)$$
The integration can indeed be over a region $\SR$ which is a subset of $\re^4$ in general. Mostly, we take $\SR = [t_1,t_2]\times \re^3$.\\[0.2cm]
...
&=\SL(\phi, \partial_\mu \phi, x^\mu) + \pdv{\SL}{\phi}\delta\phi + \pdv{\SL}{(\partial_\mu\phi)}\delta(\partial_\mu\phi)
\end{align*}
...
This is the KG equation that we had seen earlier. Thus, the above Lagrangian models the KG equation.\\[0.2cm]
When we have multiple fields $\{\phi_i\}$, then for each field, we will obtain:
$$\pdv{\SL}{\phi_i} - \partial_\mu\qty(\pdv{\SL}{(\partial_\mu\phi_i)})=0$$
% \subsection{Stress-Energy Tensor}
% The Lagrangian is of the form: 
% $$\SL(\phi_i, \partial_\mu \phi_i, x^\mu)$$
% Let us consider the derivative of the Lagrangian with respect to the coordinate $x^\mu$. We have then,
% \begin{align*}
%     \dv{x^\mu}\SL &= \pdv{\SL}{\phi_i}\pdv{\phi_i}{x^\mu} +  \pdv{\SL}{(\partial_\nu\phi_i)}\pdv{(\partial_\nu\phi_i)}{x^\mu} +\pdv{\SL}{x^\nu}\underbrace{\pdv{x^\nu}{x^\mu}}_{\tensor{\delta}{^\nu _\mu}}\\
%     &=\partial_\nu\qty(\pdv{\SL}{(\partial_\nu\phi_i)})\pdv{\phi_i}{x^\mu} + \pdv{\SL}{(\partial_\nu\phi_i)}\pdv{(\partial_\nu\phi_i)}{x^\mu}+\pdv{\SL}{x^\nu}\tensor{\delta}{^\nu _\mu}\\
%     &=\partial_{\nu}\qty(\pdv{\SL}{(\partial_\nu\phi_i)}\pdv{\phi_i}{x^\mu}) +\partial_\nu\SL\tensor{\delta}{^\nu _\mu}
% \end{align*}
% Let us define the quantity:
% $$\tensor{T}{^\mu _\nu} := \pdv{\SL}{(\partial_\mu \phi)}\partial_{\nu}\phi - \SL \tensor{\delta}{^\mu _\nu}$$
% This is a mixed \textit{tensor} of rank $(1,1)$. Note the following:
% \begin{align*}
%     \tensor{T}{^0_0} = \pdv{\SL}{(\partial_t \phi)}\partial_{t}\phi - \SL 
% \end{align*}
NOTE: In terms of the total Lagrangian, the ELEOM for the field is:
$$\fdv{L}{\phi} - \partial_t \qty(\fdv{L}{\dot{\phi}})=0$$
