...
\begin{itemize}
    ...
    \item Since the trace, that is, the sum of the eigenvalues, should equal zero, we should definitely have half of the eigenvalues as +1 and the other half to be -1. Thus, these matrices are guaranteed to be \textit{even} dimensional. 
...
\begin{align*}
  \gamma^0 &:= \beta\\
\gamma^i &:=\gamma^0\alpha_i
\end{align*}
These matrices are called \textit{Dirac matrices}. Then, using this definition in the above expression, we have:
\begin{align*}
  i \gamma^0 \partial_0\psi  +i \gamma^i \partial_i\psi - m\psi =0 \implies (i \gamma^\mu \partial_\mu - m)\psi =0
\end{align*}
Note that, we had introduced the 4-vector concept in an ad-hoc manner. However, doing this, we finally derive the well-known covariant form of the \textit{Dirac equation}!
\subsection{Form of Dirac Matrices}
The Dirac matrices that we had defined have the following form:
\begin{align}\label{gammat}
  \gamma^i = \mqty(\mathds{1}&0\\ 0& -\mathds{1}) \mqty(0&\sigma_i\\ \sigma_i& 0)  = \mqty(0&\sigma_i\\ -\sigma_i& 0)
\end{align}