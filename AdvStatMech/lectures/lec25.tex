...
\begin{equation}
   m(t,h) = \begin{cases}
    ...
    \end{cases}
\end{equation}
where $\Delta$ is a ``new'' exponent which is introduced and is often referred to as the \textit{gap exponent}. $\SF_{\pm}$ are well-behaved universal functions\footnote{So, these functions will be same for every system belonging to the same \textit{universality class}}. This is still a bit \textit{ad hoc} and we need to justify writing this expression. However, let us come to what we can do, provided this expression is valid. Firstly from the experimental data, if we plot $m/\abs{t}^\beta$ vs. $h/\abs{t}^\Delta$, then we expect that all the data across various systems will fall on the same curve. This is called \textit{data collapse} and is very useful for estimating the exponents (but we do have to ``guess'' the exponents for the collapse, since these are not known beforehand).\\[0.2cm]
