...
\begin{align}\label{eqn:potential}
    \begin{split}
        \qty(\pdv[2]{G}{P})_{T,N} &=  \qty(\pdv{V}{P})_{T,N} = -V\SK\tund{T} \leq 0\\
    \qty(\pdv[2]{F}{V})_{T,N} &= - \qty(\pdv{P}{V})_{P,N} = \frac{1}{V\SK\tund{T}} \geq 0
    \end{split}
\end{align}
...
At high temperature, the $P$ vs. $V$ curves at constant temperature (\textit{isotherm}) follows the usual ideal-gas curve. However, there exists a critical temperature $T\tund{c}$ (which can be found out from the equation), such that, for $T<T\tund{c}$, there always exist a region, where the slope $\pdv{P}{V}$ is positive. Since $\SK\tund{T} = -\frac{1}{V}\qty(\pdv{V}{P})_{N,T}$, this implies a negative compressibility, which, as motivated before, is \textit{unphysical}.\\[0.2cm]
