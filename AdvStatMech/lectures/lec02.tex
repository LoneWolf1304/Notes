...
\section*{Lecture 02: Stability of Thermodynamic Systems}
\addcontentsline{toc}{section}{Lecture 02: Stability of Thermodynamic Systems}
...
\begin{figure}[H]
    \centering\input{pics/entro_u}
\end{figure}
\noindent
Then, the resultant entropy after this transfer would be greater than the initial entropy. This would lead to inhomogenities in the system. Within one subsystem, there would be an uneven re-distribution of energy within different regions. This loss of homogenity indicates a \textit{phase separation}!\\[0.2cm]
...
Note that in the region AB and DE, entropy is still concave, hence these are called \textit{metastable} or locally stable states. These states satisfy Eq.\eqref{eqn:local} but does not satisfy Eq.\eqref{eqn:global} while the region BCD satisfy neither condition.\\[0.2cm]
\textbf{NOTE:}\\[0.5cm]
A generic condition for stability is given by $\delta P\delta V \leq 0$ where $\delta P$ and $\delta V$ represent a small deviation from the equilibrium value. We can write,
\begin{align}\label{eqn:stability}
    0 \leq \delta V \qty[\pdv{P}{V}\Bigg|_T \delta V + \frac{1}{2}\pdv[2]{P}{V}\Bigg|_T (\delta V)^2+\frac{1}{6}\pdv[3]{P}{V}\Bigg|_T (\delta V)^3+\ldots]
\end{align}
The first term has $(\delta V)^2$ which is always positive. Hence to make this term negative, we need $$\pdv{P}{V}\Big|_T\leq 0$$ Suppose that $\pdv{P}{V}\Big|_T=0$. Then the second term has $(\delta V)^3$ and irrespective of what sign $\pdv[2]{P}{V}\Big|_T$ has, $\delta V$ can always be chosen to make the overall term positive. Hence the only option we have is that $$\pdv[2]{P}{V}\Big|_T=0$$ Similar to the first term, we have the third term as, $$\pdv[3]{P}{V}\Big|_T\leq 0$$ These define the stability conditions for our thermodynamic system. 
\subsection{Stability Relations for Potentials}
We will now discuss what are the consequence of stability on the thermodynamic potentials, $F$ and $G$. We will use the fact that for a mechanically stable system, the \textit{specific heat} and the \textit{isothermal compressibility} must be positive for all temperatures. It can be motivated since compressibility and specific heat are related to number and energy fluctuations, $\sigma\tund{N}$ and $\sigma\tund{E}$ respectively, both of which are positive.\\[0.2cm]
...
\begin{align}\label{eqn:potential}
    \begin{split}
        \qty(\pdv[2]{G}{P})_{T,N} &=  \qty(\pdv{V}{P})_{T,N} = -V\kappa\tund{T} \leq 0\\
    \qty(\pdv[2]{F}{V})_{T,N} &= - \qty(\pdv{P}{V})_{P,N} = \frac{1}{V\kappa\tund{T}} \geq 0
    \end{split}
\end{align}
where we have appropriately used the definitions of isothermal compressibility and heat capacity. Using this, we find that $G$ is a concave function of pressure and $F$ is a convex function of volume. From the above expressions in Eq \ref{eqn:potential}, we can also write 
\begin{equation}\label{eqn:inverseGA}
...
At high temperature, the $P$ vs. $V$ curves at constant temperature (\textit{isotherm}) follows the usual ideal-gas curve. However, there exists a critical temperature $T\tund{c}$ (which can be found out from the equation), such that, for $T<T\tund{c}$, there always exist a region, where the slope $\pdv{P}{V}$ is positive. Since $\kappa\tund{T} = -\frac{1}{V}\qty(\pdv{V}{P})_{N,T}$, this implies a negative compressibility, which, as motivated before, is \textit{unphysical}.\\[0.2cm]
