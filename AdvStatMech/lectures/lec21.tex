...
\end{align*}
where $\SL$ is the same as in Eq. \eqref{flucland}. From this structure, we can see that if $\vb{k} \neq \vb{q}$, then the numerator and denominator gets cancelled. Thus all we are left with is just the integral over $\vb{q}$:
\begin{align*}
    &\frac{\int \dd\widetilde{\phi}_{\vb{q}}^R\dd\widetilde{\phi}^I_{\vb{q}}\ \qty[{(\widetilde{\phi}_{\vb{q}}^R)}^2+{(\widetilde{\phi}_{\vb{q}}^I)}^2] \exp\qty(-\frac{\beta}{V}\qty[\gamma q^2+a\tund{2}(T-T_c)]\qty[{(\widetilde{\phi}_{\vb{q}}^R)}^2+{(\widetilde{\phi}_{\vb{q}}^I)}^2])}{\int \dd\widetilde{\phi}_{\vb{q}}^R\dd\widetilde{\phi}^I_{\vb{q}} \ \exp\qty(-\frac{\beta}{V}\qty[\gamma q^2+a\tund{2}(T-T_c)]\qty[{(\widetilde{\phi}_{\vb{q}}^R)}^2+{(\widetilde{\phi}_{\vb{q}}^I)}^2])} \\ &\quad\quad=  \frac{\int \dd\widetilde{\phi}_{\vb{q}}^R\ {(\widetilde{\phi}_{\vb{q}}^R)}^2 \exp\qty(-\frac{\beta}{V}\qty[\gamma q^2+a\tund{2}(T-T_c)]{(\widetilde{\phi}_{\vb{q}}^R)}^2)}{\int \dd\widetilde{\phi}_{\vb{q}}^R \ \exp\qty(-\frac{\beta}{V}\qty[\gamma q^2+a\tund{2}(T-T_c)]{(\widetilde{\phi}_{\vb{q}}^R)}^2)}+\frac{\int \dd\widetilde{\phi}_{\vb{q}}^I\ {(\widetilde{\phi}_{\vb{q}}^I)}^2 \exp\qty(-\frac{\beta}{V}\qty[\gamma q^2+a\tund{2}(T-T_c)]{(\widetilde{\phi}_{\vb{q}}^I)}^2)}{\int \dd\widetilde{\phi}_{\vb{q}}^I \ \exp\qty(-\frac{\beta}{V}\qty[\gamma q^2+a\tund{2}(T-T_c)]{(\widetilde{\phi}_{\vb{q}}^I)}^2)}
\end{align*}
...
\begin{align*}
    2\times \frac{\int \dd x \ x^2 e^{-c_q x^2}}{\int \dd x \ e^{-c_q x^2}} = \frac{\kb TV}{\gamma k^2+a\tund{2}(T-T_c)}
\end{align*}
Thus, the average correlation is obtained as, 
\begin{equation}
    \avg{\abs{\widetilde{\phi}_{\vb{q}}}^2} =  \frac{\kb TV}{\gamma q^2+a\tund{2}(T-T_c)}
\end{equation}
We can also use a heuristic equipartition method to get to this in an effortless way. From equiparition we know that each degree of freedom gets a contribution of $\sfrac{\kb T}{2}$. Then, in Eq. \eqref{gausslandau} for each $\vb{k} = \vb{q}$, we have: 
$$\frac{1}{2V}\qty[\gamma q^2 + a\tund{2}(T-T_c)]\expval{\abs{\widetilde{\phi}_{\vb{q}}}^2} = \frac{1}{2}\kb T \implies \expval{\abs{\widetilde{\phi}_{\vb{q}}}^2} = \frac{V\kb T}{\gamma q^2 + a\tund{2}(T-T_c)}$$
