...
\begin{align}
    ...
    \end{split}
\end{align}
where $\mathrm{h}_{\text{\tiny eff}} = h+J\gamma \avg{\sigma}$ is kind of the \textit{mean field} which is subjected to each spin. The Hamiltonian depends on $\avg{\sigma}$, however, given the Hamiltonian, we can also calculate $\avg{\sigma}$. This leads to a \textit{self-consistent} equation which we need to tackle!
