...
\subsection{Phase Transition in 2D Ising System ( Peierls' Argument)}
...
\begin{align}
    ...
    \end{split}
\end{align} 
From the above expression, if $h=0$ then we have $\SM(0) = 0$ for all temperatures, which apparently says that \textit{phase transition is impossible in zero-field!}\\[0.2cm]
...
Hence, at all temperature $\SH$ has a symmetry under the exchange of spins but $\avg{\sigma_i}$ does not. Thus we say that the symmetry is spontaneously broken for $T<T_c$.\\[0.2cm]
