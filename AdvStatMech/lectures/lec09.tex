...
\subsection{Phase Transition in 2D Ising System ( Peirls' Theorem)}
...
and hence the sum converges if $\abs{r}<1\implies e^{\beta J}>3$. The value of the sum in the RHS is $\qty\frac{r}{(r-1)^2}$ and hence,
$$\mathrm{Prob}(\{\sigma\} \in \Omega\tund{0}) < \frac{r}{(r-1)^2} = \frac{3e^{-\beta J }}{(3e^{-\beta J }-1)^2}$$
As $\beta\to \infty$ (corresponding to $T\to 0$), $\mathrm{Prob}(\{\sigma\} \in \Omega\tund{0}) \to 0$ and hence for suciently low temperatures all spins in the systems tend to align even without an external magnetic field, proving the existence of a phase transition. 
\subsection{Symmetries of the Ising Model}
The zero-field Ising Hamiltonian has an obvious symmetry of $\sigma_i \to -\sigma_i$, which is broken if $h\neq 0$. If non-zero field, from Eq. \eqref{isingham}, we can see that if $h\to -h$ and $\{\sigma\}\to \{-\sigma\}$, then the Ising Hamiltonian remains invariant. 
...
\end{align*}
where in the second sum we used the fact that changing $\sigma\to -\sigma$ will not matter since we are summing over both $-1$ and $+1$ spins. 
