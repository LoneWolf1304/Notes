...
where $g(\lambda)$ is some function. A simple example for $p=2$ is, $f(x) = x^2$ since $f(\lambda x) = \lambda^2 x^2 = \lambda^2  f(x)$, with $g(\lambda) = \lambda^2$. Note that, $g(\lambda)$ cannot be any arbitrary function, it must satisfy some properties. For this, note: 
...
Let us now focus only on the two variable case, which is $f(\lambda x)$. The static scaling hypothesis states that the thermodynamic potentials are GHFs near the critical point $T_c$. Then the free energy can be written as, 
$$F(\lambda^{a\tund{t}}t, \lambda^{a\tund{h}}h) = \lambda F(t,h)\quad \longrightarrow \quad -\pdv{h}\qty[F(\lambda^{a\tund{t}}t, \lambda^{a\tund{h}}h)] = -\pdv{h}\qty[A(t,h)] \implies \lambda^{a\tund{h}-1} m(\lambda^{a\tund{t}}t, \lambda^{a\tund{h}}h) =m(t,h)$$
