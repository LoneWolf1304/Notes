...
$$\vb{k} = \frac{2\pi}{L} \vb{n}\qquad \vb{n} \in \inte^d$$
...
\begin{equation}
    \boxed{\SZ = \SN \int   \prod\limits_{\substack{\vb{k}\in \Omega\\ \abs{\vb{k}}<\Lambda}} \dd\widetilde{\phi}_{\vb{k}}^R\dd\widetilde{\phi}^I_{\vb{k}}\ \exp \qty[-\frac{\beta}{V}\qty[\gamma k^2 + a\tund{2}(T-T_c)][(\widetilde{\phi}^R_{\vb{k}})^2+(\widetilde{\phi}^I_{\vb{k}})^2]]}
\end{equation}
