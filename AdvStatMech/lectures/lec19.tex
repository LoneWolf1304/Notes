...
$$F = -\kb T \qty[\SZ\tund{0} +\sum\limits_{\substack{\vb{k}\in \Omega\\ \abs{\vb{k}}<\Lambda}}\ln \qty(\frac{\pi \kb T V}{\gamma k^2+a\tund{2}(T-T_c)})  ] = F\tund{0} -\frac{\kb T}{2}\sum\limits_{\abs{\vb{k}}<\Lambda}\ln \qty(\frac{\pi \kb T V}{\gamma k^2+a\tund{2}(T-T_c)})$$
