...
\begin{enumerate}
    \item Since metric is diagonal, $\sigma = \mu$ gives the only non-zero value. Since all the symbols are symmetric in lower indices, so we will calculate the value for only one such index pair.
