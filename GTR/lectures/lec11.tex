...
\end{align*}
Using this in Eq. \ref{eqn:geod1} we have:
\begin{alignat*}{3}
...
$$\cot\theta = c_1 \cos\phi+c_2\sin\phi \qquad \text{for some $c_1, c_2$}\label{eqn:gc}$$ 
...
\end{figure}
\noindent
Note however that, if we travel on the opposite arc of the great circle, it will be the longest route. This is because, geodesics are paths of \textit{extreme} action, not only minimum. In general, thus, we should never fly along the latitude (except if it is the equation, since equator is itself a great circle). Now, let us consider a cylinder.
\begin{figure}[H]
  \centering
...
Suitably taking $k=0$, we see that the equation represents a helix with pitch $2\pi m$. Thus, the geodesics on the cylinder are helices and the shortest distance to move from one point to another on the cylinder is along a helix. 