...
\begin{align}
    ...
    R_{tt} &= -\frac{1}{2f}\qty[f'' - \frac{(f')^2}{2f} - \frac{f'h'}{2h} - 2\frac{fh'}{rh}] \label{eqn:ric2} \\
    R_{tt} &= -\frac{r}{h^2f}\qty[hf - \qty(\frac{r}{h})^2 +1] \label{eqn:ric3}
\end{align}
...
where $\SC_1$ is a constant. Now we introduce some physical argument, that far away from the influence of the star, the metric should transform into the Minkowski metric (in polar coordinates obv) $$ds_{\mathrm{Mink.}}^2 = -\ \dd t^2 +\ \dd r^2+ r^2\qty(\dd \theta^2 + \sin^2\theta \ \dd \phi^2)$$ since the effect of gravity becomes negligible as the potential falls off as $\sim r^{-2}$.
Thus we have:
...
\chris{r}{\phi}{\phi} &= \sin^2\theta (r_s-r) &
