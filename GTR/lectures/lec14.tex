...
\section*{Lecture 14: Towards the action for Gravity: Riemann Tensor}
...
\begin{align}\label{eqn:chrishw}
    \partial_\mu \ln (-\detg)  = \Tr (g \partial_\mu g) = (g \partial_\mu g)^{\rho _\rho} = g^{\rho\sigma} \partial_\mu g_{\rho\sigma}
\end{align}
...
\end{itemize}
Thus, we are finally in a position to sew together all the components of the action. The function $f$ can be a function of the Ricci scalar, that is, $f\equiv f(R)$. We take the simplest possible case where $f$ is linear in $R$, that is, 
...
$$S_{\mathrm{G}} = \alpha \int \dd x^4 \sqrt{-\detg}[R- 2\Lambda]$$
This is almost fine, we just need to find something more about $\alpha$.  Note that action has dimensions of the angular momentum and the metric is dimensionless. Thus, $R$ will have dimensions $L^{-2}$ since it has two derivatives of a dimensionless quantity (metric). Thus, 
\begin{align*}
    \dim{\alpha} &= \frac{\dim{S_{\mathrm{G}}}}{[L^4] \dim{R}}=\frac{ML^2T^{-1}}{L^4 L^{-2}}=\frac{(LT^{-1})^3}{\dim{G}}=\frac{c^3}{G}
\end{align*}
...
$$S_{\mathrm{EH}} = \frac{c^3}{16\pi G}\int \dd x^4 \sqrt{-\detg}[R- 2\Lambda] =\frac{c^4}{16\pi G}\int \dd t \ \dd^3 x \sqrt{-\detg}[R- 2\Lambda] $$
