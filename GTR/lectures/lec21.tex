...
\addcontentsline{toc}{section}{Lecture 21: Orbit Equation}
...
Introducing $u\tund{0} = \sfrac{GMm^2}{l^2} = \sfrac{1}{r_0}$, we get a simplified equation:
$$\boxed{\dv[2]{u}{\phi}+u = u\tund{0}+3GMu^2}$$
...
\begin{equation}\label{eqn:idk}
    \dv[2]{u}{\phi}+u = u\tund{0}+3GM\vep u^2 \quad \longrightarrow \text{solution:} u = u\tund{N} + \vep u\tund{E}
\end{equation}
...
$$\frac{\dd^2}{\dd \phi^2}(u-u\tund{0}) +(u-u\tund{0})=0$$
whose solution is given by $u(\phi) = u\tund{0} + \SA \cos(\phi-\phi_0)$. We can take $\phi_0=0$. We write this in an alternative form as:
\begin{equation}
...
$\vartriangleright$ \textbf{GR corrections to the orbit}\\[0.2cm]
Substituting $u=u\tund{N} + \vep u\tund{E}$ as specified earlier, we have:
\begin{align*}
    \dv[2]{u}{\phi} + u\tund{N} + \vep u\tund{E} =u\tund{0}+ 3GM\vep (u\tund{N} + \vep u\tund{E})^2   \implies u\tund{0} + \vep \dv[2]{u\tund{E}}{\phi} + u\tund{E} = u\tund{0} +3GM\vep (u\tund{N} + \vep u\tund{E})^2 
\end{align*}
Upto $\SO(\vep)$, the RHS is just $3GMu\tund{N}^2 \vep$ and hence we have:
$$ \dv[2]{u\tund{E}}{\phi} + u\tund{E} = 3GMu\tund{N}^2 = 3GMu\tund{0}^2 (1+e\cos\phi)^2$$
Let us define $u\tund{E}^o :=3GMu\tund{0}^2$. Now, for many stars, the eccentricity is close to zero, so we can assume $e\ll 1$ which allows us to expand the square term linearly. We get:
\begin{equation}\label{eqn:ue}
    \dv[2]{u\tund{E}}{\phi} + u\tund{E} = u\tund{E}^o(1+2e\cos\phi)
\end{equation}
Assume an ansatz of $u_E =  u\tund{E}^o + b\phi \sin\phi$ and note that:
\begin{align*}
...
\begin{align*}
    b (2\cos\phi-\phi\sin\phi+\phi\sin\phi)+ u\tund{E}^o=  u\tund{E}^o(1+2e\cos\phi) \implies \boxed{b=e u\tund{E}^o}
\end{align*}
Then upto $\SO(\vep)$ the equation of orbit in GR $(\vep=1)$ becomes:
\begin{equation}\label{eqn:grrad}
    \frac{1}{r(\phi)} =  u\tund{N}+  u\tund{E} = u\tund{0}(1+e\cos\phi) + \qty[3GMu\tund{0}^2(1+e\phi\sin\phi)]
\end{equation}
...
\begin{align*}
    \dv{u}{\phi} = -u\tund{0}e\sin\phi +3GMu\tund{0}^2e\qty(\sin\phi + \phi\cos\phi)\Bigg|_{\phi_n}=0
\end{align*}
Substituting the value of $\phi_n$ and noting that $\sin(2n\pi+x) = x$, we get:
\begin{align*}
    \sin(\delta\phi)(1-3GMu\tund{0}) - 3GMu\tund{0}(2n\pi+\delta\phi)\cos(\delta\phi) = 0
\end{align*}
Upto $\SO(\delta\phi)$, we obtain $\sin(\delta\phi) \approx \delta \phi$ and $\cos(\delta\phi)\approx 1$. Substituting this above we get:
$$\delta\phi(1-6GMu\tund{0}) = 6GMu\tund{0} n\pi \implies \delta\phi =\frac{6GMu\tund{0}}{(1-6GMu\tund{0})}n\pi \simeq \frac{6GM^2m^2}{l^2}n\pi$$
...
Note the potential for GR, which has an attractive $\frac{1}{r^3}$ term which dominates for $r\to 0$ and thus, there exists a point of no return, that is, if $r<r_o$, then we cannot come back as $V^{\text{eff}}\tund{GR}$ is unbounded from below. 
\begin{figure}[H]
    \centering 
...
Note that blackhole is possible only if $R<r_s$, that is, there exists a limit for the radius of the object. If $R>r_s$, then the solution no longer remain an `exterior' solution and all the assumptions become invalid since now, $\ST_{\mu\nu}\neq 0$ for $r<r_s<R$. If $M=M_{\odot}$ (where $M_{\odot}$ denotes the \textit{solar mass}), then blackhole is only possible if,
$$\frac{2GM_\odot}{Rc^2}>1 \implies R<\frac{2GM_\odot}{c^2} = \frac{2\times 6.67\cdot 10^{-11}\times 2\cdot 10^30}{(3\cdot 10^8)^2}\approx 2964\ \text{m}$$
Thus, if our Sun could be condensed to less than $3 \ \mathrm{km}$, then it would have been a black-hole.
