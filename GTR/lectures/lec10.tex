...
Since the primed frame is locally inertial, the metric is \textit{Minkowski}, that is,$$\eta_{\mu\nu}=g'_{{\mu}{\nu}} = \pdv{x^\alpha}{x'^\mu}\pdv{x^\beta}{x'^\nu}g_{{\alpha}{\beta}}$$
...
\begin{equation}\label{eqn:chris_met}
    \chris{\lambda}{\mu}{\nu} =\frac{1}{2}g^{\rho\lambda} \qty(\partial_\mu g_{\rho\nu}+\partial_\nu g_{\mu\rho}-\partial_\rho g_{\mu\nu})
\end{equation}
