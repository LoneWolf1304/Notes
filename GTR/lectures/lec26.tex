...
$\triangleright$ \textbf{Christoffel Symbols}\\[0.2cm]
...
$\triangleright$ \textbf{Riemann and Ricci Tensor}
\begin{align*}
...
\begin{align*}
    ...
    &=\qty(\partial_t \chris{t}{x}{x}-\cancelto{0}{\partial_x \chris{\rho}{x}{\rho}} + 3\chris{x}{x}{t}\chris{t}{x}{x}-\chris{t}{x}{x}\chris{x}{t}{x}-\chris{x}{x}{t}\chris{t}{x}{x})\\
    &=\dv{(a\dot{a})}{t} + 3\qty(\sfrac{\dot{a}}{a})(a\dot{a} ) -2\qty(\sfrac{\dot{a}}{a})(a\dot{a} ) \\
    &=a\ddot{a} + \dot{a}^2 +\dot{a}^2 =  a\ddot{a}  +2\dot{a}^2 
\end{align*}
By isotropy we can say that $R_{xx}= R_{yy}=R_{zz} =a\ddot{a} + 2\dot{a}^2$\\[0.2cm]
$\triangleright$ \textbf{Ricci Scalar}
\begin{align*}
...
$$\ST^{\mu\nu} = (P+\rho)u^\mu u^\nu + Pg^{\mu\nu}$$
The macroscopic speed of the fluid cannot have a privileged direction, so it must have only a temporal component: $\fvec{u} = (u^t,0,0,0)$. Now the norm of the velocity four vector is always $-1$
$$g_{\mu\nu}u^\mu u^\nu = g_{tt}(u^t)^2 = -(u^t)^2 = -1 \implies u^t=1 \quad u_t = -1$$
$\triangleright$ First Einstein Equation \\[0.2cm]
