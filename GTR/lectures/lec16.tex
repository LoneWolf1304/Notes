...
\begin{align*}
    ...
    \hbar&=1 \implies \frac{6.626\times 10^-34 \ \mathrm{kg-m}^2 }{3\times 10^8\ \mathrm{ m}} = 1 \implies 1\ \mathrm{ m } \approx 2\times 10^{42} \ \mathrm{ kg}^{-1}
\end{align*}
...
\begin{identity}[Jacobi Identity]
    ...
    $$x\times (y\times z) + z\times (x\times y) + y\times (z\times x) = 0 \iff x\times (y\times z) + \mathrm{cyc}. (x,y,z)$$
\end{identity}
The Jacobi identity is satisfied by many binary operations, like the cross-product, the commutator brack, the Poisson bracket, etc\footnote{All these are particular cases of a broaded thing called \textit{Lie Bracket}}. We will consider the Jacobi identity for the commutator of the covariant derivatives here!
