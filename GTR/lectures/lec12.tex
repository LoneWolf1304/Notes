...
\begin{figure}[H]
    \centering
    \input{pics/jacob.tex}
\end{figure}
\noindent
In Cartesian coordinates, the volume element is $\dd V = \dd x \ \dd y$ while in the polar coordinates, the volume elements is $\dd V' = \dd r \ (r\ \dd \theta) = r \ \dd \theta \ \dd r$
...
From this, we can derive another formula for the determinant using the Lev-Civita symbol:
\begin{identity}
...
This is indeed the expression for the volume element. The above formula is related to differential forms and wedge products which are defined in terms of \textit{alternating-operator} and gives a generalisation of the volume element $\dd V = dx^1\wedge dx^2 \wedge \ldots$.\\[0.2cm]
Anyway, let us now calculate something. Since we want $\dd V = \dd V'$, then we demand that:
\begin{align*}
