...
$$\Ss[\psi, g] = \Ss\tund{EH}[g] + \Ss\tund{M}[\psi,g]$$
where $\Ss\tund{M}$ denotes the action for any matter field $\psi$, like electromagnetic field, fermionic and bosonic fields, etc.
...
Consider a planet of mass $m$ orbiting around a star of mass $M\gg m$ and radius $R$ at the origin. In the Newtonian case, the gravitational force and the corresponding potential  is given by $$\tvec{F} = -\frac{GMm}{r^2}\hat{\vb{r}}\qquad V_{\mathrm{N}} = -\frac{GMm}{r}$$
\begin{figure}[H]
    \centering 
...
\begin{equation}
    \boxed{ds^2 = f(r)\ \dd t^2 +h(r)\ \dd r^2+ r^2\qty(\dd \theta^2 + \sin^2\theta \ \dd \phi^2)} \label{eqn:metric}
\end{equation}
